...
export async function loadIndexSource(input, options = {}) {
...
export function parseMystSearchIndex(raw, source = 'myst.search.json') {
    let parsed;
    try {
...
async function fetchRemoteIndex(indexUrl) {
    let response;
    try {
    ...
    if (!response.ok) {
        throw new Error(`Could not fetch ${indexUrl}. Received HTTP ${response.status}. Check that this is a MyST site with myst.search.json published.`);
    }
